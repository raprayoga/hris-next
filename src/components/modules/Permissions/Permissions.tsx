...
import { deletePermission, getPermission } from "@/services/permissionService";
...
export default function Permissions({
  ...
  const getPermissionList = async () => {
    try {
      ...
      dispatch(
        showToast({
          message:
            error?.response?.data?.message ?? "Failed to get permissions list",
          type: "danger",
        })
      );
      setError(error);
  ...
  const handleDeleteItem = async () => {
    try {
      await deletePermission(idDelete);
      dispatch(
        showToast({
          message: "Success to delete permissions",
          type: "green",
        })
      );
      ...
      dispatch(
        showToast({
          message:
            error?.response?.data?.message ?? "Failed to delete permissions",
          type: "danger",
        })
      );
      setError(error);
  ...
  return (
    <>
          ...
          <tbody>
            {permissions.length === 0 && (
              <tr className="text-center">
                <td colSpan={4}>Permission Not Found</td>
              </tr>
            )}
            {permissions.length > 0 &&
              permissions.map((permission) => (
                <tr className="border-b bg-white" key={permission.id}>
      ...
      >
        ...
        <p className="text-sm">Be sure to delete this permission ?</p>
        <p
