...
import { EditPermission, Permission } from "@/interface/permission";
...
export function EditPermissionForm({
  ...
  const ispermission: string = router.query.id as string;
  ...
  const onSubmit: SubmitHandler<EditPermission> = async (data) => {
    setIsLoading(true);
    try {
      await updatePermission(+ispermission, data);
      dispatch(
        showToast({
      ...
      dispatch(
        showToast({
          message:
            error?.response?.data?.message ?? "Failed to add new permission",
          type: "danger",
        })
      );
      setError(error);
  ...
  const getEditPermission = async () => {
    try {
      const permission = await editPermission(+ispermission);
      setPermission(permission.permission);
    } catch (error: any) {
      dispatch(
        showToast({
          message: error?.response?.data?.message ?? "Permission not valid",
          type: "danger",
        })
      );
      setError(error);
  ...
  return (
    <div {...props} className={cn("text-center", className)}>
      {permission && permission.id === +ispermission && (
        <form
